...
def default_output_path(output_directory: Path, model_name: str) -> Path:
    repository = MODEL_CONFIGS[model_name]["repository"]
    model_directory = re.sub(r"[^A-Za-z0-9._-]+", "-", repository.rsplit("/", maxsplit=1)[-1]).strip(".-")
    directory = output_directory / model_directory
    versions = [int(match.group(1)) for path in directory.glob("dense_retrieval_evaluation_v*.jsonl") if (match := re.fullmatch(r"dense_retrieval_evaluation_v(\d+)\.jsonl", path.name))]
    return directory / f"dense_retrieval_evaluation_v{max(versions, default=0) + 1}.jsonl"
...
def main() -> None:
    ...
    metadata = {
        "run_number": None if arguments.output else int(re.search(r"_v(\d+)\.jsonl$", output_path.name).group(1)),
