    ...
    async def _stream_traced(
        ...
        if explicit_memory_request:
            response_text = (
                ...
                else "Dugoročna memorija je isključena. Uključite je u podešavanjima da biste sačuvali preferenciju."
                if explicit_memory_request == "disabled" and language == "sr"
                else "Long-term memory is disabled. Enable it in Settings to save a preference."
                if explicit_memory_request == "disabled"
